...
import { createPostInput, createComment } from "@repo/common/config";
 ...
 * COMMENT ROUTES
 */
blogRoutes.post("/auth/comment", async (c) => {
  const prisma = new PrismaClient({
    datasourceUrl: c.env.DATABASE_URL,
  }).$extends(withAccelerate());
...
blogRoutes.put("/auth/comment/:id", async (c) => {
  const id = c.req.param("id");
  const prisma = new PrismaClient({
    datasourceUrl: c.env.DATABASE_URL,
  }).$extends(withAccelerate());
...
blogRoutes.delete("/auth/comment/:id", async (c) => {
  const id = c.req.param("id");
...
blogRoutes.post("/auth/comment/like/:id", async (c) => {
  const id = c.req.param("id");
  ...
  const comment_like = await prisma.commentLikes.create({
    data: {
      commentId: id,
      userId: jwt_response?.id,
    },
